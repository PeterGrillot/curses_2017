...
function processUrls(urls, tweetObj) {
  urls.forEach((urlObj, index) => {
    ...
    var finalText = quotedTweetHtml || urlObj.display_url.link(urlObj.url);
...
function processMedia(media, tweetObj) {
  media.forEach((mediaObj) => {
    if(mediaObj.type === 'photo') {
      var image = '<img src="' + mediaObj.media_url_https + '"/>';
      ...
      var video = '<video controls poster="' + mediaObj.media_url_https +'">' + source + '</video>';
